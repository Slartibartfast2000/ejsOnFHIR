...
router.get("/everything", async (req, res) => {
  ...
  try {
    const response = await fetch(`http://localhost:8080/fhir/Patient/${id}/$everything`);
    const everything = await response.json();
    console.debug(everything);
    const entries = everything.entry;
    res.render('./partials/Search/searchResourceResults', { entries });

   // res.status(200).json({ message: 'everything successfully', entries });
